...
class ImageDataGenerator(object):
	...
	if not isinstance(self.channel_shift_range,(list,)):
	    self.channel_shift_range = [self.channel_shift_range]
	if len(self.channel_shift_range)!=3:
	    self.channel_shift_range = self.channel_shift_range*3
        if any(self.channel_shift_range):

            x = random_channel_shift(x, np.asarray(self.channel_shift_range),img_channel_index)
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
                 ...
                 dim_ordering='default', bbox_util=None,
                 classes=None, class_mode='categorical',
    ...
    def next(self):
	    ...
	    if self.image_data_generator.preprocessing_function is not None:
		if self.image_data_generator.preprocessing_function is 'rgb2hsv':
